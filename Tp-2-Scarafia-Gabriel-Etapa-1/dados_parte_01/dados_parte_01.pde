...
void dibujarGrilla(int espacio){
  ...
  // -- tú código para armar la grilla
    for (int y = -h/2; y < h/2; y += espacio) {
    line(-w/2, y, w/2, y);
    }
    for (int x = -w/2; x < w/2; x += espacio) {
      line(x, -h/2, x, h/2);
    }
  // *** FIN ***
}
...
void mouseDragged() {
  ...
  float velocidadRotacionCamara = 0.05;
  anguloCamera += deltaX * velocidadRotacionCamara;
  anguloCamera = constrain(anguloCamera, -PI, PI);
  // *** FIN ***
